...
**The problem is a disclosure problem.** In a Korean 다가구주택 — one owner, one title register, many
...
**hatched band and never a level** — the page genuinely does not know the total, and a precise surface
would be either a lie or a disclosure.

Note that `issueCertificate` is deliberately **not** gated on the registry's secret key — it takes no
...
just is the ability to open the commitment**, which is exactly the property the attack demo
exercises.
...
This is the point of the whole project. A bare commitment would be useless — the registry could pick
...
**Where the refusal happens, stated precisely.** A landlord who understates the total fails at
**circuit-execution time, on their own machine** — before a proof exists, before anything is
...
protect.** Sending that to a third-party prover would leak precisely the thing being protected, so
